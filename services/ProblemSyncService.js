	...
	client.get(url, function(data, response,done){
		...
		problems.insert(_problems	 , function(err , success){
			...
			if (err){
				...
				io.emit('syncUpdate', {status:_statusERROR,from:_syncName,timestamp:_timestamp,info:err.message,type:type});
				...
				io.emit('syncUpdate', {status:_statusSUCCESS,from:_syncName,timestamp:_timestamp,info:_problems.length+" items",type:type});
			...
			io.emit('syncUpdate', {status:_statusERROR,from:_syncName,timestamp:_timestamp,info:_message,type:type});
			_syncStatus.saveLastSync(_syncName,_timestamp,_message,_statusERROR,type);
	})
}
